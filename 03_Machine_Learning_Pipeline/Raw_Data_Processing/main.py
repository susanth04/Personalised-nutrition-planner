...
class MicrobiomeDataProcessor:
    ...
    def load_specific_files(self):
        ...
        try:
            ...
            self.genera_counts = pd.read_csv(r"C:\RESEARCH-PROJECT\03_Machine_Learning_Pipeline\Data_Integration_Pipeline\genera.counts (2).tsv", sep="\t")
            print(f"Loaded genera_counts: {self.genera_counts.shape}")
            
            self.metadata = pd.read_csv(r"C:\RESEARCH-PROJECT\03_Machine_Learning_Pipeline\Data_Integration_Pipeline\metadata (1).tsv", sep="\t")
            print(f"Loaded metadata: {self.metadata.shape}")
            
            self.mtb = pd.read_csv(r"C:\RESEARCH-PROJECT\03_Machine_Learning_Pipeline\Data_Integration_Pipeline\mtb.tsv", sep="\t")
            print(f"Loaded mtb (metabolites): {self.mtb.shape}")
            
            self.species_counts = pd.read_csv(r"C:\RESEARCH-PROJECT\03_Machine_Learning_Pipeline\Data_Integration_Pipeline\species.counts (2).tsv", sep="\t")
            print(f"Loaded species_counts: {self.species_counts.shape}")
            
            self.species = pd.read_csv(r"C:\RESEARCH-PROJECT\03_Machine_Learning_Pipeline\Data_Integration_Pipeline\species (4).tsv", sep="\t")
            print(f"Loaded species: {self.species.shape}")
            
            self.genera = pd.read_csv(r"C:\RESEARCH-PROJECT\03_Machine_Learning_Pipeline\Data_Integration_Pipeline\genera (1).tsv", sep="\t")
            print(f"Loaded genera: {self.genera.shape}")
            
            self.lda = pd.read_csv(r"C:\RESEARCH-PROJECT\03_Machine_Learning_Pipeline\Data_Integration_Pipeline\LDA_bacteria.csv", sep="\t")
    ...
    def create_unified_dataset(self):
        ...
        def safe_merge(base_df, df_to_merge, prefix):
            ...
            # Check if there's a common sample pattern that can be extracted
            # For example, if there's a subject ID that's common across datasets
            if 'Subject' in base_df.columns and any('Subject' in col for col in df_to_merge.columns):
                # Try matching on Subject instead of Sample
                subject_col = next(col for col in df_to_merge.columns if 'Subject' in col)
                return pd.merge(
                    base_df,
                    df_to_merge.reset_index(),
                    left_on='Subject',
                    right_on=subject_col.replace(f"{prefix}_", ""),
                    how='left'
                )
            
            # If no common identifier, you may need to use a lookup table or mapping
            # For this example, I'll create a demonstration of using a Sample mapping
            # In practice, you'd need to create this mapping based on your data
            
            print(f"Warning: No direct match found for {prefix} data. Using best-effort mapping.")
            
            # Option 1: If datasets are in the same patient order but different ID formats
            # This assumes the row order is meaningful (same patients in same order)
            if len(base_df) == len(df_to_merge):
                print(f"Datasets have same number of samples ({len(base_df)}). Using position-based alignment.")
                df_to_merge = df_to_merge.reset_index(drop=True)
                base_df_reset = base_df.reset_index()
                result = pd.concat([base_df_reset, df_to_merge], axis=1)
                if 'Sample' in result.columns:
                    return result.set_index('Sample')
                else:
                    return result
            
            # Option 2: Use a subset of matching samples if available
            base_samples = set(base_df.index)
            merge_samples = set(df_to_merge.index)
            common_samples = base_samples.intersection(merge_samples)
            
            if common_samples:
                print(f"Found {len(common_samples)} common samples. Using only these for {prefix} data.")
                filtered_base = base_df.loc[base_df.index.isin(common_samples)]
                filtered_merge = df_to_merge.loc[df_to_merge.index.isin(common_samples)]
                return pd.merge(
                    filtered_base,
                    filtered_merge,
                    left_index=True,
            ...
            # Last resort: Just add empty columns with the right names
            print(f"Warning: Could not align {prefix} data with base dataset. Adding empty columns.")
